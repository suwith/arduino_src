...
// get a distance reading from the USS
  ...
  if(dist_raw == 0)
    dist_raw = lastdist_raw;
  else lastdist_raw = dist_raw;
...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  else if(dist_raw == dist_avg ){
    analogWrite(PIN_LED, 0);
  }
  else if(dist_raw == (dist_min + dist_avg)/2|| dist_raw == (dist_avg + dist_max)/2){
    analogWrite(PIN_LED, 127);
  }
  else analogWrite(PIN_LED, 191);
